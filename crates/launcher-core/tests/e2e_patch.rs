...
use launcher_core::config::{Config, PatchConfig};
...
async fn end_to_end_patch() {
    ...
    let exe_path = launcher_dir.join("BossPatcher_e2e.exe");
    let config_path = launcher_dir.join("BossPatcher_e2e.toml");
    ...
    let emitter: Arc<Mutex<CollectingEmitter>> =
        Arc::new(Mutex::new(CollectingEmitter::default()));
    let patcher = Patcher::new();
    let result = patcher
        .run(&launcher_dir, &exe_path, &config_path, &config, emitter.clone())
        .await
    ...
    assert!(
        tokio::fs::try_exists(launcher_dir.join("extra_remote.txt"))
            .await
            .unwrap()
    );
...
async fn prepare_remote_files() -> Vec<RemoteFile> {
    vec![
        ...
        RemoteFile {
            path: "BossPatcher_e2e.exe".into(),
            contents: b"Protected launcher".to_vec(),
        },
        RemoteFile {
            path: "BossPatcher_e2e.toml".into(),
            ...
            loop {
                ...
                tokio::spawn(async move {
                    let service = service_fn(move |req| {
                        handle_request(req, files.clone(), manifest.clone())
                    });
